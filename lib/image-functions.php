...
function kibble_custom_thumbnail_html( $html, $post_id, $post_image_id ) {
	...
	if (empty( $html )) {
		$html = '<img width="120" title="'. $title .'" height="160" src="http://fakeimg.pl/120x160/282828/eae0d0/" class="thumbnail img-responsive" />';
	}
	$html = preg_replace(array('/(alt|title)=\"[^"]*"\s/','/class=\"[^"]*"\s/', '/width=\"[^"]*"\s/','/height=\"[^"]*"\s/'),array('title="'. $title .'" ','class="img-responsive thumbnail" ','width="180" ', 'height="240" '),trim($html));
    	return $html;
}
